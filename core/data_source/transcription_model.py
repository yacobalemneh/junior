...
class TranscriptionModel:
    ...
    def load_model(self, model_type: ModelType):
        ...
        if model_type not in self.models:
            self.logger.info(f"Loading model (TranscriptionModel): {model_type}")
    ...
    def unload_model(self, model_type: ModelType):
        if model_type in self.models:
            self.logger.info(f"Unloading model (TranscriptionModel): {model_type}")
            del self.models[model_type]
        

    def switch_current_model(self, model_type: ModelType):
        self.logger.info(f"switch_current_model called with model_type: {model_type}")
        self.current_model = self.models[model_type]
        self.logger.info(f"current_model set to : {model_type} by switch_current_model")

    def transcribe(self, audio):
        if self.current_model is None:
            raise ValueError("Model not loaded.")
        audio = self.pad_audio(audio)
        result_dict = self.current_model.transcribe(audio, fp16=False, language='English')
