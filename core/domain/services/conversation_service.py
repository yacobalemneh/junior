...
import logging
import time
from core.domain.services.hotword_detection_service import DetectionResult
...
class ConversationService:
    ...
    def start_conversation(self, audio_source, audio_processor):
        ...
        while True:
            ...
            if self.hotword_detection_interactor.listen_for_hotwords(processed_audio) == DetectionResult.HOTWORD:
                ...
                if self.stopword_detected:
                    complete_audio = audio_processor.process(audio_source.buffer)
                    result = self.transcription_service.transcribe(complete_audio)
                    logging.warning(f'Transcription result in listening: {result.text}') 
        ...
        ## Line below to trigger the audio source to stop streaming 
